...
export default async function getIntegralElection(req, res) {
	if (req.method === 'POST') {
		const data = req.body;
		...
		if (election.success) {
			const response = {
				...
				constituencies: election.election.constituencies,
				ballots: election.election.ballots.length,
				ballotBox: election.election.ballotBox,
				electionTabulating: election.election.electionTabulating,
				shuffles: election.election.shuffles,
				electionComplete: election.election.electionComplete,
				electionResults: election.election.electionResults,
			};
